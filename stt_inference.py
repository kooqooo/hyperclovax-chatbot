...
async def atranscribe_audio(file_io: io.BytesIO, model_name="maxseats/SungBeom-whisper-small-ko-set9"):
    device = "cuda" if torch.cuda.is_available() else "cpu"
...
async def atranscribe_audio_with_model(
    file_io: io.BytesIO,
    model: WhisperForConditionalGeneration,
    processor: WhisperProcessor,
    device: str = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    async def process_file(file_io: io.BytesIO):
        # Byte 데이터를 waveform으로 변환
        waveform, sample_rate = torchaudio.load(file_io)
    ...
    transcription = await process_file(file_io)
    return transcription
